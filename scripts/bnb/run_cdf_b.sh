...
for ((i=1; i<=INSTANCES; i++)); do
  ...
  for ((j=0; j<=5; j++)); do
    ...
    for ((k=0; k<=9; k++)); do
      echo "Run Nr $k"
      mpirun -np $MPI_PROCESSES --bind-to core --map-by :OVERSUBSCRIBE  build/mallob $MY_MALLOB_OPTIONS >scripts/bnb/out_b/"file_"$i"_"$MPI_PROCESSES"_"$k".txt"
    done
  done
done 
